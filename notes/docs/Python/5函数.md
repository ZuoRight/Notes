...
函数执行完return语句就结束了，但函数执行完yield语句只是暂停

调用含有yield的函数，返回对象是一个生成器
...
gen = demo()
print(gen)  # <generator object demo at 0x100d55a50>
...
使用 `yield from` 可以调用另一个 yield 函数：<https://iswbm.com/471.html>
