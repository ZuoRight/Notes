...
for…in…语句后面可以跟if条件筛选，注意，不能带else。

`expression for item in iterable if condition`

表达式中可以包含if...else...判断。

`expression if condition else for item in iterable if condition`

可以包含多层循环。一般就两层，内层在前面，外层在后面

`expression1 for item1 in iterable1 expression2 for item2 in iterable2`
