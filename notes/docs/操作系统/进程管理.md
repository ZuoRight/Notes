...
程序(Program)，通常为二进制文件，一般存储在物理磁盘中，被用户触发执行后会加载到内存中，以进程(Process)的方式运行，为了管理这个进程，操作系统会根据触发这个进程的用户与相关属性关系，给予这个进程一组有效的权限，以及进程所需要的脚本或数据，最后再给予一个PID，操作系统通过通过PID来判断该进程是否有执行权限。
...
daemon主要分为本地服务和网络服务，网络服务会提供一个端口(port)，供外部客户端请求连接
