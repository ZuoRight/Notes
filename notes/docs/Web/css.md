...
分辨率就是屏幕中显示的像素个数，比如 1920×1080 意味着水平方向含有 1920 个像素数，垂直方向含有 1080 个像素数，屏幕尺寸相同的情况下，分辨率越高，显示效果越细腻。

CSS 中的各种单位最终都会转化为像素（pixel）的基本单位：`px`

### 绝对单位

```text
1 in(英寸) = 2.54 cm(厘米) = 6pc(派) = 72 pt(点) = 96px(像素)
```
